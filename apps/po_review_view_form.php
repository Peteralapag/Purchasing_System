...
$userpreveledgeapprover = checkPolicy($username, $applications, $module, 'p_review', $db);
...
<div class="po-wrapper">
	...
	<div class="no-print d-flex justify-content-between align-items-center">
	    ...
	    <div>
	        <button class="btn btn-danger btn-sm" onclick="voidPO('<?= $po['po_number'] ?>','<?= $po['pr_number'] ?>','<?= $userpreveledgeapprover?>')">
	...
	<div style="display:flex; justify-content:space-between; margin-top:40px; align-items:flex-start;">
	    ...
	    <div style="width:30%;">
	        <div style="border-top:1px solid #000; padding-top:5px; text-align:center">
	            Prepared by<br>
	            <strong><?= htmlspecialchars($po['created_by']) ?></strong>
	        </div>
	    </div>
	        ...
	        <?php if ($po['status'] == 'PENDING'): ?>
				...
				<div class="no-print" style="display:flex; justify-content:center;">
				    <button class="btn btn-success btn-sm"
				        onclick="reviewdPO('<?= $po['po_number'] ?>','<?= $po['pr_number'] ?>','<?= $userpreveledgeapprover?>')">
				        <i class="fa fa-check-circle"></i> Confirm Review
				    </button>
				</div>				
...
function reviewdPO(poNumber, prNumber, userprev) {

	if (userprev == 0) {
        swal({
		    title: "Access Denied",
		    text: "You do not have permission to review this purchase order.",
		    icon: "warning",
		    button: "OK"
		});
		return false;
    ...
    swal({
        ...
        text: "Are you sure you want to mark this PO as reviewed?",
        icon: "warning",
        buttons: true,
        dangerMode: false,
    })
    .then((willReview) => {
        ...
        fetch('./Modules/Purchasing_System/actions/po_actions.php', {
            ...
            body: 'mode=reviewpo&ponumber=' + encodeURIComponent(poNumber)
        })
        .then(res => res.text())
        .then(data => {
        ...
        .catch(err => {
            console.error(err);
            swal("Error", "Error marking PO as reviewed.", "error");
        });
    });
}
