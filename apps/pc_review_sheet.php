...
<div style="border:1px solid #000; padding:20px; font-family:Arial; font-size:13px; max-height:83vh; overflow-y:auto">
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="form-check">
                                <input class="form-check-input approve-radio" type="radio" name="approve-<?= $item['id'] ?>" value="SELECTED" <?= $s['status'] === 1 ? 'checked' : '' ?> disabled>
    ...
    <table width="100%" style="margin-top:40px">
        <tr>
            ...
            <td><b>REVIEWED BY:</b><br><br>
                <?php if(empty($header['reviewed_by'])): ?>
                    <button class="btn btn-sm btn-outline-success" onclick="reviewCanvass('<?= $canvass_no ?>')"><i class="fa fa-check-circle"></i> Review</button>
                <?php else: ?>
                    <?= htmlspecialchars($header['reviewed_by']) ?>
                <?php endif; ?>
            </td>
...
function bactomain(){
    $('#contents').load('./Modules/Purchasing_System/includes/pc_review.php');
}
